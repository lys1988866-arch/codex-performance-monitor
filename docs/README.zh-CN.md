...
默认只读。日志相关按钮只会修改本机 Codex 日志数据库：截断 WAL、安装 TRACE/DEBUG 拦截。优化内存按钮只会请求 Windows 修剪工作集内存，不会结束进程。结束进程按钮只会结束你明确选中并确认的单个进程。它不会安装或重装 Codex，也不会修改你的 Codex 项目。

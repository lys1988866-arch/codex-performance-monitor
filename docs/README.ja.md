...
- リスク スコアと理由の表示。
- 実行できる対応手順の表示。
- 監視対象プロセスのワーキング セットを修剪してメモリを最適化。
- 選択したプロセスを確認後に終了。
- 選択した PID のコピー。
...
既定では読み取り専用です。ログ操作ボタンはローカルの Codex ログ SQLite データベースだけを変更します。メモリ最適化は Windows にワーキング セットの修剪を依頼するだけで、プロセスを終了しません。プロセス終了は、選択して確認した単一プロセスだけに実行されます。Codex をインストール/再インストールしたり、Codex プロジェクトを変更したりしません。
